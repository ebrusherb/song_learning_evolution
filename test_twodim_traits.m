...
%     sum(x)
    ...
    z=popdyns(11:12,t);
    F=prefs.*repmat(reshape(x,1,[]),4,1);
    F=F./repmat(sum(F,2),1,8);
%     F=F./repmat([1+alpha*sum(x([1 2 5 6])); 1+alpha*sum(x([3 4 7 8])); 1+alpha*sum(x([1 3 5 7])); 1+alpha*sum(x([2 4 6 8]))],1,8);
    F=repmat([col(y); col(z)],1,8).*F;
%     sum(sum(F))

...
examples[[i]] <- ggplot(dist,aes(x=mrange,y=dist,color=f_sigma2)) + geom_line() + 
	...
	scale_color_manual(values=c('white',eightpal[fvals-1]))+
...
for(i in 1:lm){
	...
	toplot = melt(log(sqrt(var_mat_m[,2:Nfs,m,p]),base=10),varnames = c('sigma2','f_sigma2'))
	toplot$sigma2 = as.factor(rep(sigma2_vals,times=(Nfs-1)))
	toplot$f_sigma2 = log(rep(fmix_sigma2_vals[2:Nfs],each=Ns),base=10)
	colnames(toplot)[3] = 'y'
	toplot$size = as.vector(num_peaks[,2:Nfs,m,p])
	...
	bubble[[i]] <- ggplot(toplot, aes(x=f_sigma2, y=y, size=size, color = sigma2),guide=FALSE)+
		...
		scale_color_manual(values=eightpal)+
		labs(color=expression(sigma),size='# of peaks') +
		 scale_x_continuous(limits=range(log(fmix_sigma2_vals,base=10))+c(-.4,.1),breaks=log(xbreaks,base=10),labels=xbreaks)+

...
\vspace{.1 in}
\noindent {\color{red} CODE FOR THE POPULATION DYNAMICS:}
...
{\color{red} INTERESTING PARAMETERS:} 
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
Even with a continuous distribution of female preferences, we find that after several generations, the male song distribution is concentrated in several discrete peaks (Figure \ref{fig:plot_peaks}). 
\begin{knitrout}
...
\hlstd{pop_dens2} \hlkwb{=} \hlkwd{dynamics}\hlstd{()}
\hlstd{Pm2} \hlkwb{=} \hlstd{pop_dens2}\hlopt{$}\hlstd{Pm}
\hlstd{Pf2} \hlkwb{=} \hlstd{pop_dens2}\hlopt{$}\hlstd{Pf}

\hlcom{# break down the mating and preference probabilities}
\hlstd{c} \hlkwb{=} \hlnum{1e-10} \hlcom{#recognition cutoff}
\hlstd{d} \hlkwb{=} \hlkwd{sqrt}\hlstd{(}\hlopt{-}\hlnum{2}\hlopt{*}\hlstd{sigma2}\hlopt{*}\hlkwd{log}\hlstd{(}\hlkwd{sqrt}\hlstd{(}\hlnum{2}\hlopt{*}\hlstd{pi}\hlopt{*}\hlstd{sigma2)}\hlopt{*}\hlstd{c))}
\hlcom{#^distance / difference at which a female can recognize a male}
...
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{Nm)\{}
        ...
        \hlstd{recognized[i]} \hlkwb{=} \hlkwd{int}\hlstd{(Pf2[}\hlkwd{intersect}\hlstd{(w1,w2),}\hlnum{1}\hlstd{]}
        \hlopt{*}\hlkwd{dnorm}\hlstd{(x}\hlopt{-}\hlstd{frange[}\hlkwd{intersect}\hlstd{(w1,w2)],}\hlkwc{mean}\hlstd{=}\hlnum{0}\hlstd{,}\hlkwc{sd}\hlstd{=}\hlkwd{sqrt}\hlstd{(sigma2)))}
        \hlcom{#^how many females recognize each male, weighted by their preferences}
\hlstd{\}}

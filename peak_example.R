...
plot(mrange[w1]+1,p1$z[w1,t],t='l',lwd=lwd,col='black',xlab='',ylab='',ylim=range(c(p1$z[w1,t],p2$z[w2,t])))
points(mrange[w1]+1,p2$z[w1,t],t='l',lwd=lwd,col=col_vec[1],xlab='',ylab='')
mtext('Preference',side=1,line=1.5,at=0,cex=largefontsize/smallfontsize)
mtext('Z',side=2,line=1.7,at=mean(range(c(p1$z[w1,t],p2$z[w2,t]))),cex=largefontsize/smallfontsize)

# plot(mrange[w1]+1,(apply(p1$pxy[w1,,t],1,sum)-p1$Pm[w1,t])/p1$Pm[w1,t],t='l',lwd=lwd,col='black',xlab='',ylab='',ylim=range((apply(p2$pxy[w2,,t],1,sum)-p2$Pm[w2,t])/p2$Pm[w2,t],na.rm=TRUE),yaxt='n')
# axis(2,at=c(-.1,0,0.2,.4),labels=c(-.1,0,0.2,.4))
# lines(mrange[w2]+1,(apply(p2$pxy[w2,,t],1,sum)-p2$Pm[w2,t])/p2$Pm[w2,t],t='l',lwd=lwd,col=col_vec[1],xlab='',ylab='')
# mtext('Song',side=1,line=1.5,at=0,cex=largefontsize/smallfontsize)

...
p=2
m=4
toplot = melt(log((smat[,2:Nfs,m,p])),varnames = c('sigma2','f_sigma2'))
toplot$f_sigma2 = rep(fmix_sigma2_vals[2:Nfs],each=Ns)
toplot = cbind(toplot,as.vector(log(sqrt(var_mat_m[,2:Nfs,m,p]))))
toplot = cbind(toplot,step*(as.vector(num_peaks[,2:Nfs,m,p]+.05)))
colnames(toplot)[3:5] = c('x','y','size')
toplot$f_sigma2 = as.factor(toplot$f_sigma2)

bubble <- ggplot(toplot, aes(x=x, y=y, size=size, color = f_sigma2),guide=FALSE)+
	geom_point()+ scale_size_area(max_size = 25,limits = step*c(0,max(four_freq)))+
	geom_line(size=1)+
	scale_color_manual(values=mypal)+
	labs(color=expression(sigma[f])) +
	scale_x_continuous(name="Log(flexibility)")+
	scale_y_continuous(name="Log(standard deviation)",limits=c(-6,.5))+
...
# m=3;radius=sqrt(step*(as.vector(four_freq[,1:Nfs,m,p]+.05)));symbols(log(as.vector(smat[,1:Nfs,m,p])),log(sqrt(as.vector(var_mat_m[,1:Nfs,m,p]))),circles=radius,bg=(as.vector(colmat[,1:(Nfs),m,p])),inches=.5*max(radius),xlab='Selectivity',ylab='Standard deviation');ylim=par('yaxp')[1:2];legend(x=-1,y=mean(ylim,),legend=(fmix_sigma2_vals[1:Nfs]),lty=0,pch=16,col=(mypal[1:(Nfs)]),bty='n');for(f in 1:Nfs){abline(h=log(fmix_sigma2_vals[f]),col=mypal[f])}

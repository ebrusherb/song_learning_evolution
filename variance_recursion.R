...
# sigmaxvals = seq(0,3,by=0.05)
# xx = length(sigmaxvals)
# sigmayvals = seq(0,3,by=0.05)
...
# layout(matrix(c(1,2),ncol=2));
# breaks=c(0,seq(0.1,1000,length.out=20))
# image(sigmaxvals,sigmayvals,final_x,breaks=breaks,col=c('black',heat.colors(length(breaks)-2)));
# abline(0,1,col='blue')
# image.scale(final_x,horiz=FALSE,breaks=breaks,col=c('black',heat.colors(length(breaks)-2)))
...
# sigmax = 8
# c = 3
# # poss=polyroot(c((c-1)*sigma^4,0,(2*c-1)*sigma^2-sigmay^2,0,c))
# # solved = max(Re(poss[which(abs(Im(poss))<=1e-14)]))
# # sigmax = solved-1
# sigmax = (0.5773503)#-0.1
# sigmay = sqrt(0.8)*sigmax
# sigmax = 4
# sigmay = 0.5*sigmax
# F = c()

# for(t in 1:30){
	# F = c(F,(sigma^2*(sigma^2+sigmax[t]^2)+sigmax[t]^2*sigmay[t]^2)/(sigma^2+sigmax[t]^2)^2)
	# sigmax = c(sigmax,sqrt(sigmax[t]^2/4*((sigma^2*(sigma^2+sigmax[t]^2)+sigmax[t]^2*sigmay[t]^2)/(sigma^2+sigmax[t]^2)^2+1)))
	# sigmay = c(sigmay,sqrt(sigmax[t]^2*(sigma^2*(sigma^2+sigmax[t]^2)+sigmax[t]^2*sigmay[t]^2)/(sigma^2+sigmax[t]^2)^2))
# }

# # sigma=0.6
# sigmaxvals = seq(0,2,by=0.05)
# sigmayvals = seq(0,2,by=0.05)
# solved=array(NA,length(sigmayvals))
# for(i in 1:length(sigmayvals)){
	# sigmay_now=sigmayvals[i]
	# poss=polyroot(c(2*sigma^4,0,5*sigma^2-sigmay_now^2,0,3))
	# hold = max(Re(poss[which(abs(Im(poss))<=1e-14)]))
	# if(length(hold)==0){hold=NA}
	# solved[i] = hold
# }
# g =expand.grid(x=sigmaxvals,y=sigmayvals)
# Fmat = with(g,(sigma^2*(sigma^2+x^2)+x^2*y^2)/(sigma^2+x^2)^2)
# contour(sigmaxvals^2,sigmayvals^2,matrix(Fmat,nrow=length(sigmaxvals)),levels=c(0.01,1,seq(0,1,by=0.2),seq(2,20,by=2)),xlab='Sigmax2',ylab='Sigmay2')
# # abline(h=sigmay[1],col='red')
# # lines(solved,sigmayvals,col='red')
# # abline(0,sqrt(3),col='blue')
# # points(sigmax,sigmay,t='o',col='red')
# abline(sigma^2,1,col='red')
